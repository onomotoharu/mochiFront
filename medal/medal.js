...
// 	debugger;
	for(i=0 ; i<4 ; i++){
	  ...
	  $(this).append('<img src="medal_img/'+medal[i].picture_name+'">')
	  .attr('title', 'medal[i].description.split(" ")[0]');
	  }
	  //バッジ名を挿入
	  $(".medallog").append('<p class="b">'+medal.title+'</p>');
	  //メダル取得の日付を挿入
	  $("#getdate").append(medal.created_at.split(" ")[0]); 
	　//バッジ名を追加 
	  $(".medalintro").append(medal.description);
